...
void updateMasterCounter() {
  ...
  flow_data_payload[6] = masterCounter;
}
...
void updateTemperatureData() {
  ...
  flow_data_payload[3] = (int)temperatureF;
  memcpy(&flow_data_payload[4],  &temperatureF, sizeof(temperatureF));
}
...
void publishJsonData() {
  ...
  jsonDoc["Temperature F(int)"]= flow_data_payload[3];
  jsonDoc["Temperature F"]     = temperatureF;
  jsonDoc["Counter"]           = flow_data_payload[6];
  jsonDoc["FW Version"]        = flow_data_payload[7];
...
void printFlowData() {
  ...
  Serial.printf("%f ", *((float *)&flow_data_payload[4]));
  for (int i = 6; i <= 7; ++i) {
    Serial.printf("%x ", flow_data_payload[i]);
  }
  Serial.printf("\n");
